...
export function generateSupplyCurve(params, yAxisMax, step = 0.05) {
  const {
    utility = 100,
    iWeight = 1,
    hWeight = 1,
  ...
  for (let wageRate = wMin; wage <= wMax; wage += step) {
    // Use numeric.js to solve for t in calUtility(t) = utility
    const func = (t) => -calUtility(t, wageRate, unearnedIncome, iWeight, hWeight) ;
    const roots = numeric.zerofunc(func, 0, LEISURE_HOURS, 1e-6);
    let t;
    if (roots.length === 0) {
      // calculate t=0 or t=LEISURE_HOURS
      const u0 = calUtility(0, wageRate, unearnedIncome, iWeight, hWeight);
      const uL = calUtility(LEISURE_HOURS, wageRate, unearnedIncome, iWeight, hWeight);
      t = u0 > uL ? 0 : LEISURE_HOURS;
    } else {
      t = roots[0];    
    }
    data.push([t, wageRate]);
  }
...
export function testOpt() {
  ...
  const func = (t) => -calUtility(t, wageRate, unearnedIncome, iWeight, hWeight) ;
    ...
    console.log('Optimal leisure time t:', t);
}


function calUtility(t, wageRate, unearnedIncome, iWeight, hWeight) {
